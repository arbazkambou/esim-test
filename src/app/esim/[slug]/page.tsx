...
import { getCountriesThatHavePackages, getPackagesOfCountry } from "@/services/packages/dataOnlyPackages";
...
//this is a next js api to dynamically generate meta data for a specifc route
export async function generateMetadata({ params }: PropsType) {
  let packagesResponse;
  try {
    packagesResponse = await getPackagesOfCountry(params.slug);
  } catch (error) {
    if (
  ...
  return generateDynamicSeo({
    ...
    meta_description,
    meta_keywords,
    url: `https://esimcard.com/esim/${params.slug}/`,
...
async function Page({ params }: PropsType) {
  // this api is called to get countries that have data only packages
  ...
  return (
    <>
      ...
      {/* this component is used to display packages of country */}
      <section className="container mt-16">
        <CountryPackagesSection packages={packagesResponse} />
      </section>

      {/* animated three steps component  */}
      <EsimSteps
