  ...
  return (
    <Sheet>
      ...
      <SheetContent className="overflow-auto">
        ...
        <div className="mt-16 flex h-full flex-col items-start gap-6">
          {navLinks.map((item, index) => (
            <Link
              key={index}
              href={item.href}
              className={`rounded-pill px-3 font-sans text-sm font-500 transition-colors duration-500 ease-in-out hover:bg-primary hover:text-background`}
            >
              {item.label}
            </Link>
          ))}
          ...
          {dropdownLinks.map((item, index) => (
            <Collapsible key={index}>
              <CollapsibleTrigger className="group flex items-center gap-1 border-none px-3 font-sans text-sm font-500">
                {item.label}{" "}
                <ChevronDown
                  size={18}
          ...
          {isAuthLoading ? (
            ...
            authLinks.map((item, index) => (
              <Link
                key={index}
                href={item.href}
                className={`flex items-center gap-2 rounded-pill bg-muted px-3 py-2 font-sans text-sm font-500 transition-colors duration-500 ease-in-out ${item.label === "Logout" ? "hover:bg-destructive" : "hover:bg-primary"} hover:text-background`}
            ...
            nonAuthLinks.map((item, index) => (
              <Link
                key={index}
                href={item.href}
                className="flex items-center gap-2 rounded-pill bg-muted px-3 py-2 font-sans text-sm font-500 transition-colors duration-500 ease-in-out hover:bg-primary hover:text-background"
              >
                <User size={16} />
                {item.label}
              </Link>
