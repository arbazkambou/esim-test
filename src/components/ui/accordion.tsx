...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "group flex flex-1 items-center justify-between rounded-b-lg rounded-t-lg border-t bg-muted px-6 py-5 text-left text-sm font-medium transition-all data-[state=open]:rounded-b-none [&[data-state=open]>svg]:rotate-180",
        className,
      )}
      {...props}
    >
