...
import { Coverage } from "@/types/packages/data-only/DataOnlyCountryPackages";
...
function SimCoverageModal({ coverage }: { coverage: Coverage[] }) {
  ...
  if (searchQuery) {
    filterCoverageByQuery = reshapedCoverage.filter((item) =>
  ...
  return (
    <Drawer repositionInputs={false}>
      ...
      >
        <div className="flex flex-col gap-6">
          <div
            className={`grid gap-4 ${reshapedCoverage.length > 2 && "xl:grid-cols-2"} h-full shrink-0`}
          >
            <p className="text-xl font-500">Supported Countries & Networks</p>
