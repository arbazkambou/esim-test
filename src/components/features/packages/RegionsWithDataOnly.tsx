...
import RegionImageCard from "@/components/my-ui/cards/RegionImageCard";
import { getContinentsThatHavePackages } from "@/services/packages/dataOnlyPackages";

async function RegionsWithDataOnly() {
  const regions = await getContinentsThatHavePackages();

  const customRegions = regions.map((item) => {
    const { name } = item;
    if (name === "Africa") {
  ...
  return (
    <section className="mt-16 grid gap-10 sm:grid-cols-2 xl:grid-cols-3">
