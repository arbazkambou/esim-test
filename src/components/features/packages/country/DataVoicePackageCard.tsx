...
import { Calendar, Globe, MessageCircle, Phone, Trophy } from "lucide-react";
import Image from "next/image";
import GlobalPackageDetailModal from "../global/GlobalPackageDetailModal";
...
function DataVoicePackageCard({
  ...
  packageDetail: GlobalPackage | CountryPackage;
  index: number;
}) {
  const {
    data_unit,
  ...
  return (
    <Card
      className={`group relative rounded-md border-2 shadow-none transition-all hover:border-primary ${index === 1 && "rounded-t-none"} hover:cursor-pointer hover:shadow-xl ${value === packageDetail.id && "border-primary shadow-xl"}`}
      onClick={() => handleValueChange(packageDetail.id)}
    >
      {/* Best Choice Badge */}
      ...
      <div className="relative flex flex-col gap-2 p-3">
        ...
        {packageDetail.discount_percentage > 0 && (
          <div
            className={`absolute right-0 top-0 w-max rounded-bl-sm rounded-tl-sm rounded-tr-sm bg-primary-accent p-1 px-4 ${index === 2 ? "rounded-tr-none" : "rounded-tr-[0.9rem]"}`}
          >
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="flex items-center gap-1 text-xs leading-[100%] text-muted-foreground">
            ...
            {providers.includes(provider) ? "Unlimited SMS" : `Incoming Only`}
          </p>
          <p className="flex items-center gap-1 text-xs leading-[100%] text-muted-foreground">
        ...
        <div>
          <GlobalPackageDetailModal packageDetail={packageDetail} />
        </div>
      </div>
    </Card>
  );
