...
import WhyEsimCard from "../cards/WhyEsimCard";
    ...
    <section className="container mt-16 bg-background">
      <div className="flex flex-col gap-[1.31rem] xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex items-center justify-center">
          <Link href={"/esim"}>
            <Button className="group flex items-center gap-3 text-sm hover:text-primary">
              <ArrowUpLeft
      ...
      <div className="mt-[3.69rem] grid gap-x-[4rem] gap-y-[4rem] md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3">
        {cardsData.map((item, index) => (
          <WhyEsimCard cardData={item} key={index} />
        ))}
      </div>
    </section>
  );
